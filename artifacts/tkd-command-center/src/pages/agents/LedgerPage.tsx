...
/* ─── QuickBooks Online P&L (live, month-to-date) ────────────────
   Shape returned by GET /api/quickbooks/pnl (normalizeProfitAndLoss). */
interface QboCategory { name: string; amount: number; section: string; }
interface QboPnl {
  revenue: number;
  cogs: number;
  grossProfit: number;
  ...
  netIncome: number;
  netMargin: number;
  byCategory: QboCategory[];
  period: { start: string; end: string };
}

interface ExpenseRow { label: string; value: number; }

/* Fallback shown only until QuickBooks is connected (or if /pnl errors). */
const QB_EXPENSES_FALLBACK = {
  total: 3180,
  rows: [
...
export function LedgerPage() {
  ...
  const [qbo, setQbo]                 = useState<QboPnl | null>(null);
  ...
  const loadData = useCallback(async () => {
    setDataLoading(true);
    /* HouseCall Pro (operations) and QuickBooks (accounting) load in parallel;
       each owns its own state so a QBO failure never blocks the HCP data. */
    const hcp = (async () => {
      try {
    ...
    const qb = (async () => {
      try {
        const res  = await fetch("/api/quickbooks/pnl?period=month");
        const json = await res.json() as QboPnl & { error?: string };
        setQbo(res.ok && !("error" in json) ? json : null);
      } catch { setQbo(null); }
    })();
    await Promise.all([hcp, qb]);
      ...
      <div className="flex flex-col gap-4 md:h-[calc(100vh-108px)]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4 flex-shrink-0">
          {kpis.map((k) => (
            <div
              key={k.label}
              className={`bg-white rounded-lg px-3 py-3 md:px-4 md:py-3.5 ${k.onClick ? "cursor-pointer transition-shadow hover:shadow-md" : ""}`}
              style={{ border: "1px solid rgba(0,0,0,0.07)", boxShadow: "0 1px 4px rgba(0,0,0,0.06)", borderTop: `2px solid ${k.color}` }}
              onClick={k.onClick}
            >
              <p className="text-[9px] md:text-xs font-medium uppercase mb-1" style={{ color: "#6b7a90", letterSpacing: "0.6px" }}>{k.label}</p>
              <p className="text-2xl font-bold tracking-tight mb-0.5" style={{ color: "#1a2333" }}>{k.value}</p>
              <p className="text-[10px] md:text-xs font-medium leading-tight" style={{ color: "#64748b" }}>{k.sub}</p>
            </div>
          ))}
        </div>
          ...
          >
            ...
            <div className="flex items-center gap-3 px-5 py-3.5 flex-shrink-0" style={{ borderBottom: "1px solid #f0f0f0" }}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs" style={{ color: "#6b7a90" }}>AI Financial Advisor · HouseCall Pro{qbo ? " + QuickBooks" : ""}</p>
              </div>
            </div>
