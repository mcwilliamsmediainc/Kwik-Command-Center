...
/* ─── Mock open quotes (estimates detail API not connected) ─── */
const OPEN_QUOTES = [
...
export function ReactivationPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Use job-derived dormant customers (more accurate than filtering customer list)
      const dormant = (data.dormant_customers ?? []).slice(0, 20);
      setWinBack(dormant);
      const recoverableTotal = dormant.reduce((s, c) => s + c.totalSpent, 0);
      setKpis({ dormant: data.dormant_365, recoverable: recoverableTotal, quotes: data.estimates_count });
    } catch { /* non-fatal */ }
          ...
          <div className="px-5 py-3.5 flex items-center justify-between" style={{ borderBottom: "1px solid #f0f0f0" }}>
            ...
            {!dataLoading && (
              <span className="text-xs" style={{ color: "#6b7a90" }}>
                Top {winBack.length} by LTV · 12mo+ dormant
              </span>
            )}
          </div>
            ...
            <div className="divide-y divide-gray-50">
              {winBack.map((c) => (
                <div key={c.id} className="px-5 py-3.5 flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold truncate" style={{ color: "#1a2333" }}>{c.name}</p>
                    <p className="text-xs" style={{ color: "#6b7a90" }}>
                      {c.lastJobService ? `Last: ${c.lastJobService.slice(0, 28)}${c.lastJobService.length > 28 ? "…" : ""}` : "No recent jobs"} · Dormant {fmtDormant(c.daysSince)}
                    </p>
                  </div>
                  <span className="text-xs font-semibold flex-shrink-0" style={{ color: "#6b7a90" }}>
                    {c.totalSpent > 0 ? fmtCurrency(c.totalSpent) : "—"} LTV
                  </span>
                  <button
